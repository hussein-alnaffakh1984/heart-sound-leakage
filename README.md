...
├── .gitignore
├── src/                      Source code (add your Kaggle scripts here)
├── splits/                   Train/validation/test split indices (add here)
...
1. Install dependencies:
   ...
   ```
2. Download the PhysioNet/CinC 2016 (and PASCAL) data from the sources above.
3. Run the scripts in `src/` in order (data preparation → training → evaluation → figures). The exact train/validation/test split indices are provided in `splits/` so that results can be reproduced exactly.
